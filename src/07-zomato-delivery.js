...
export function placeOrder(restaurant, items) {
  ...
  return new Promise((resolve, reject) => {
    if(typeof restaurant !== "string" || restaurant.length === 0 || items.length === 0){
...
export function confirmOrder(order) {
  // Your code here
  return new Promise((resolve, reject) => {
    if(!order.hasOwnProperty("orderId") || !order.hasOwnProperty("status") || order.status !== "placed"){
...
export function assignRider(order) {
  // Your code here
  return new Promise((resolve, reject) => {
    if(!order.hasOwnProperty("status") || order.status !== "confirmed"){
      reject(new Error("Order not confirmed yet!"));
    }else{
      const riderPool = ["Rahul", "Priya", "Amit", "Neha", "Vikram"]
      const selectedRider = riderPool[Math.floor(Math.random() * riderPool.length)];
      resolve({
        ...order, 
        rider: selectedRider, 
        status: "assigned"
...
export function deliverOrder(order) {
  // Your code here
  return new Promise((resolve, reject) => {
    if(!order.hasOwnProperty("status") || order.status !== "assigned"){
